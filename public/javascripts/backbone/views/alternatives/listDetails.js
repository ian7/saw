/*global App,Backbone,jQuery,_,notifier,eventer,layout,nicEditor */
...
App.Views.AlternativeDetailsView = Backbone.Marionette.ItemView.extend({
		...
		decisionTable: function(param) {
			...
			for (decisionId in that.decisions) {
				...
				if (!(your_decision && your_decision.name)) {
					...
					h += "<div class='button decide " + decision.color.toLowerCase() + "' id='" + decision.decision_tag_id + "'>" + decision.name + "(" + decision.count + ")</div>";
				} else {
	...
	initialize: function() {
		...
		this.itemAttributesWidget = new App.Views.ItemWidget({
			model: this.model
		});
	...
	onRender: function() {
		...
		this.decisionListWidget = new App.Views.DecisionListWidget({
			model: this.model,
			el: jQuery("div.decisionsDetails", this.el)
		});
	...
	editRationale: function() {
		//debugger
		if (this.model && this.model.attributes.your_decision && this.model.attributes.your_decision.decision_tag_id) {
			var rationaleWidget = new App.Views.RationaleWidget({
				model: this.model
			});
...
*/
	...
	selectAll: function(e) {
		if (e.toElement.innerText === '(edit to add)' || e.toElement.innerText === '(empty)') {
...
*/
		...
		jQuery("table.decisions", this.el).block({
			message: null
		});
	},
	...
	notifyEvent: function(data) {
		...
		if (d.id === this.model.get('id')) {
			...
			var fuEl = jQuery("span.altenrnativeFocusedUsers", this.el)[0];
			if (fuEl) {
				fuEl.innerText = "";
	...
	editedAttribute: function(e) {
		...
		if (e.keyCode === 13 && !e.shiftKey) {
			newValue = e.srcElement.innerHTML;
			...
			newValue = e.srcElement.innerText;
			if (newValue === "(edit to add)" || newValue === "(empty)") {
	...
	editedRationale: function(e) {
		...
		if (e.keyCode === 13) {
			...
			var element = jQuery(e.srcElement);
...
App.Views.AlternativeDetailsWidget = Backbone.Marionette.CompositeView.extend({
	...
	onRender: function() {
        ...
        */
		var buttons = new App.Views.AltenrativeListSpeedButtons({
			collection: this.collection,
			mainView: this
		});
		layout.speedButtonsSidebar.show(buttons);

		var issueDetails = new App.Views.IssueDetails({
			id: this.issue.id,
			el: jQuery("div#issueAttributes", this.el)
		});
		issueDetails.render();
	...
	filter: function() {
		...
		for (viewId in this.children) {
			...
			for (property in itemView.model.attributes) {
				if (itemView.model.attributes[property] && typeof(itemView.model.attributes[property]) === "string" && itemView.model.attributes[property].toLowerCase().match(filter)) {
					matchedFilter = true;
				}
			}
...
App.Views.AltenrativeListSpeedButtons = Backbone.Marionette.View.extend({
	...
	newAlternative: function() {
		...
		jQuery(this.el).oneTime(1200, 'some_focus', function() {
			jQuery("div.editable#name")[1].focus();
		});
	},
	reuseAlternative: function() {
		//alert('to be implemented');
		...
		_(this.model.get('decisions')).each(function(decisionType) {
			_(decisionType.details).each(function(userDecision) {
				...
				if (this.collection.get(userDecision.decision_id)) {
					decisionModel = this.collection.get(userDecision.decision_id);
					decisionModel.set('rationale', userDecision.Rationale);
				}

				decisionModel = new Backbone.Model();
	...
	onRender: function() {
		...
		if (jQuery("div#rationaleText", this.el)[0]) {
			...
			jQuery(this.el).oneTime(300, 'some_focus', function() {
				jQuery("div#rationaleText")[0].focus();
			});
		}
	},
	save: function() {
